...
module.exports = {
  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'promote':
        text = (chat.sPromote || this.spromote || conn.spromote || '@user ```Ahora eres admin```')
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user ```Ya no eres admin```')
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: 'Éste comando solo puede ser utilizado por _*WILD*_',
    owner: 'Éste comando solo puede ser utilizado por _*WILD*_!',
    mods: 'Éste comando solo puede ser utilizado por _*ADMINS*_ !',
    premium: 'Éste comando es solo para miembros _*PREMIUM*_ !',
    group: 'Éste comando solo se puede usar en grupos.!',
    private: 'Éste comando solo se puede usar en el chat privado!',
    admin: 'Éste comando es solo para administradores del grupo.',
    botAdmin: 'Haz que el bot sea *Admin* para usar éste comando!',
...
fs.watchFile(file, () => {
  fs.unwatchFile(file)
  console.log(chalk.redBright("Archivo actualizado: 'handler.js'"))
